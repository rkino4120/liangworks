<script lang="ts">
  // ThrelteとThree.jsから必要なモジュールをインポートします
  import { T, useTask } from '@threlte/core';
  import * as THREE from 'three';
  import { onMount, onDestroy } from 'svelte';

  // --- Props ---
  // コンポーネントの外から設定を渡せるようにpropsを定義します
  export let pointCount = 5000;
  export let radius = 4.5;
  export let color = "#00d0ff";
  export let surroundingPointCount = 2000; // 周囲の点の数を減らしました

  // --- Shader Code ---

  // 頂点シェーダー: 各点の位置をGPUで計算します
  const vertexShader = `
    // 外部から渡されるデータ (Uniforms)
    uniform float uTime;
    uniform float uPointSize;
    uniform float uScatter;
    uniform float wSphere;
    uniform float wCube;
    uniform float wTorus;
    uniform vec2 uMouse;

    // 各点に固有のデータ (Attributes)
  ...
  const geometry = (() => {
    ...
    const torusMajorRadius = radius * 0.8;
    const torusMinorRadius = radius * 0.4;

    // 球体、トーラス、拡散の位置を計算
    const torusPointsPerRow = Math.round(Math.sqrt(pointCount)); // トーラスのグリッド計算用
    for (let i = 0; i < pointCount; i++) {
        ...
        torusPos[idx + 1] = (torusMajorRadius + torusMinorRadius * Math.cos(v)) * Math.sin(u);
        torusPos[idx + 2] = torusMinorRadius * Math.sin(v);
    ...
    for (let face = 0; face < 6; face++) {
        for (let j = 0; j < pointsPerRow; j++) {
            for (let k = 0; k < pointsPerRow; k++) {
                ...
                switch (face) {
                    case 0: cubePos[idx] = u; cubePos[idx+1] = v; cubePos[idx+2] = halfSize; break;
                    case 1: cubePos[idx] = u; cubePos[idx+1] = v; cubePos[idx+2] = -halfSize; break;
                    case 2: cubePos[idx] = halfSize; cubePos[idx+1] = u; cubePos[idx+2] = v; break;
                    case 3: cubePos[idx] = -halfSize; cubePos[idx+1] = u; cubePos[idx+2] = v; break;
                    case 4: cubePos[idx] = u; cubePos[idx+1] = halfSize; cubePos[idx+2] = v; break;
                    case 5: cubePos[idx] = u; cubePos[idx+1] = -halfSize; cubePos[idx+2] = v; break;
                }
                pointIndex++;
            }
            if (pointIndex >= pointCount) break;
  ...
  const transformDuration = 18; // サイクル時間を延長
  ...
  const handleMouseMove = (event: MouseEvent) => {
    ...
    surroundingUniforms.uMouse.value.x = uniforms.uMouse.value.x;
    surroundingUniforms.uMouse.value.y = uniforms.uMouse.value.y;
  };
  ...
  useTask(() => {
    ...
    // 新しいアニメーションサイクル (トーラスを含む)
    if (cycle < 0.15) { // Sphere
        wSphere = 1;
